...
import { fetchProduct,deleteProduct,updateProduct, fetchVendorProduct } from '@/app/helpers/backend';
...
const page = () => {
    const [data, getData, { loading }] = useFetch(fetchVendorProduct);
    ...
    return (
        <div>
            ...
            action={
                <Button
                onClick={() => {
                    router.push('/vendor/inventory/add')
                }}
            >
                {i18n.t("Add New")}
            </Button>
            ...
            onEdit={(values) => {
                router.push(`/vendor/inventory/edit/${values?._id}`)
            }}
            onReload={getData} 
            loading={loading} 
